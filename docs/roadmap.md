...
| Now      | Workflow parity and release ([milestone v0.9.0](https://github.com/akoita/draft-loop/milestone/4)) | [Seven observations of one consented case are indeterminate](consented-pilot-v0.9.md); parity not validated | Demonstrate the complete application-grade workflow and publish evidence              | Run #291 with the declared user-session request timeout after #290 exhausted its default-timeout attempts; #75 and #250 remain blocked |
...
Issue #291 owns one fresh same-scope observation with the existing configurable
request timeout explicitly set to the declared 20-minute case budget. Keep #75
unvalidated and leave #250 blocked pending that result.
