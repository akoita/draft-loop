...
| Workspace retrieval and policy     | Partial integration                   | Workspace-scoped SQLite FTS/BM25 supplies selected chunks to live requests. Explicitly selected writing policy content and structured forbidden-term/punctuation rules are versioned in run context, supplied to both model roles, and checked deterministically with content-free locations. Dedicated policy history/migration, opportunity overrides, shared editors, and application-grade drafting remain open.                                                                                         |
| Opportunity brief                  | Implemented application components    | A strict provider-independent contract and process-local application intake assemble approved URLs, selected files, pasted content, and candidate instructions into immutable draft/review versions with honest provenance and visible failures. Persistence/restart recovery, provider extraction, shared CLI/desktop adapters, and exact reviewed-version planning handoff remain open.                                                                                                                         |
...
The next #67 component adds process-local application intake for multiple
...
Persistence/restart recovery, structured extraction, shared CLI/desktop
adapters, and exact reviewed-version planning handoff remain open. #67 and the
milestone remain incomplete.

The next #70 component extends the existing explicitly selected workspace
policy with backward-compatible structured forbidden-term and punctuation
rules. Normal orchestration applies the exact policy frozen into run context,
and deterministic findings identify the violated rule and artifact location
without copying sensitive terms or draft text. Dedicated policy-version
history and migration, anti-formulaic defaults, locale/page/verbosity/order and
emphasis preferences, opportunity-bound overrides, and shared CLI/desktop
editing remain follow-up work. #70 and the milestone remain incomplete.
