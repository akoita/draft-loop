...
| Opportunity brief                  | Implemented application components    | A strict provider-independent contract and application intake assemble approved URLs, selected files, pasted content, and candidate instructions into immutable draft/review versions with honest provenance and visible failures. Workspace-scoped SQLite persistence supports checksum-verified restart recovery and stale-version protection. Provider extraction, shared CLI/desktop adapters, and exact reviewed-version planning handoff remain open.                                                  |
...
content-free version metadata. Structured extraction, shared CLI/desktop
