...
| Product CKB workflow               | Integrated control foundation         | Shared CLI/desktop controls create, open, select, rename, and archive CKBs without projecting roots through the renderer; archive has an explicit confirmation boundary. Read-only adapters expose bounded path-free source/version summaries, duplicate groups, lifecycle readiness, and count-only structural inventory. Explicit single-file and approved URL intake use the shared application contract; desktop file selection stays host-owned, and URL results omit URLs and content. Workspace bindings pin store/CKB identities and record freshly validated source/version selections in new run contexts, with fail-closed drift checks. Retrieval, directory intake, later-version mutation, deletion, backup/restore, and provider use remain outside the product path. |
...
single-file and approved URL intake. They do not satisfy the stage without
remaining source intake/mutation controls, retrieval enforcement, and drafting
integration.
...
- Retrieval and index-version enforcement of bound CKB selection snapshots,
  normalized facts, directory intake, later-version source mutation, source
