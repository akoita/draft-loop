...
| Product CKB workflow               | Integrated control foundation         | Shared CLI/desktop controls create, open, inspect, select, rename, and archive CKBs without projecting roots through the renderer; archive has an explicit confirmation boundary. Workspace bindings pin store/CKB identities and record freshly validated source/version selections in new run contexts, with fail-closed drift checks. Retrieval, remaining source controls, deletion, backup/restore, and provider use remain outside the product path. |
