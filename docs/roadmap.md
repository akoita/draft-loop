...
| Now      | Author model revalidation ([milestone](https://github.com/akoita/draft-loop/milestone/16))                       | #499 indeterminate; [#501 route diagnosis](evaluation/author-route-diagnosis.md): 0 of 6 synthetic requests failed | Test whether `claude-sonnet-5` produces an accepted draft with the v3 prompt           | Size and duration do not reproduce the `api_error`; the production author schema is the next discriminating synthetic test |
