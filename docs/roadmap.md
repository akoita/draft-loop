...
| Portable CKB                       | [Released Sprint 2 checkpoint](stage-evidence-v0.7.0-alpha.2.md); post-checkpoint deletion implemented; v0.7 incomplete/unvalidated | The separate local store has coordinated writers, recoverable owned writes, explicit six-class retention planning, portable backup/restore, and exact-plan deletion for archived non-default CKBs. Shared CLI/desktop application contracts keep sensitive origins private, and deletion preserves unknown or unowned data. |
