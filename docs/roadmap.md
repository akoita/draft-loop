...
| Now      | Author model revalidation ([milestone](https://github.com/akoita/draft-loop/milestone/16))                       | [#499 indeterminate](evaluation/author-model-revalidation.md): `claude-sonnet-5` returned `api_error`, then timed out; no draft | Test whether `claude-sonnet-5` produces an accepted draft with the v3 prompt           | The route decision (larger time budget, route diagnosis, or deterministic structured blocks) returns to the user |
