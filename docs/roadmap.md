...
| Product CKB workflow               | Integrated binding foundation         | An explicit local workspace binding pins store/CKB identities and records a freshly validated, path-free source/version selection in every new immutable run context. Provider-capable operations fail closed when current lifecycle evidence differs from that record. Normalized facts, CKB-scoped retrieval, adapter controls, index-version enforcement, repair, deletion, backup/export/restore, and provider use remain outside the product path. |
