...
| Portable CKB                       | [Released checkpoint](stage-evidence-v0.7.0-alpha.1.md); v0.7 incomplete/unvalidated | The separate local store has logical identity, source/version provenance, managed bytes, local-only origins, URL provenance, retirement markers, and a prospective write journal. Shared CLI/desktop application contracts expose explicit selection plus file, URL, and bounded directory lifecycle controls without projecting sensitive origins.                                                                                              |
