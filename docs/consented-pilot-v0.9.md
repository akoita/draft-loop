...
The twenty-first observation (#378) followed the exact merged revision that
includes #376, but one non-retryable Anthropic author attempt failed with code
`unknown` before a draft. It did not exercise or validate #376's corrected
cumulative-usage acceptance in live use. No artifact or independent review
existed, so #378 adds no product-quality evidence and does not change the
indeterminate result.

The twenty-second observation (#382) used the exact revision, which included
issue #380. All three Anthropic author attempts ended in generic `invalid-response`:
the first two were retryable, while the third was non-retryable with
...
remain blocked. That authorization did not extend to later live attempts.

## Bounded matched-backend observation (#362)

On 2026-09-12, explicit authorization for provider transmission preceded
execution. The observation reused the same private matched-backend case,
baseline-withheld v1 gate, model pair, and fixed limits as #350, #358, and #360.
Anthropic `claude-sonnet-4-5` remained the author and OpenAI `gpt-5.6-luna` the
critic through user-session authentication; both probes passed.
...
All three Anthropic author executions failed with the generic
`invalid-response` code. Attempts one and two were retryable; attempt three was
non-retryable and exhausted the three-attempt cap. The final durable run record
has `failureStage` and `failureReason` set to
`output-token-budget-exceeded`, with one content-free diagnostic. Persisted
`activeDurationMs` was 1,033,665. Each execution record has zero input, output,
and total tokens and `estimatedUsd: null`. The workspace `totalCostUsd` was
persisted as `0`, but provider-reported cost was unavailable; this value is not
an actual provider-cost measurement.
...
remains indeterminate, and #75 and #250 remain blocked. That authorization did
not extend to later live attempts.

## Bounded matched-backend observation (#364)

On 2026-09-12, after Claude reauthentication, explicit authorization for
provider transmission preceded execution. The observation reused the same
private matched-backend case, baseline-withheld v1 gate, model pair, and fixed
limits as #350, #358, #360, and #362. Anthropic `claude-sonnet-4-5` remained the
author and OpenAI `gpt-5.6-luna` the critic through user-session authentication;
both probes passed.
...
All three Anthropic author executions failed with the generic
`invalid-response` code. Attempts one and two were retryable; attempt three was
non-retryable and exhausted the three-attempt cap. Attempt two was observed
with `failureStage` and `failureReason` set to `factual-invariant-rejection`,
with eight content-free diagnostics. The final durable run record has
`failureStage` and `failureReason` set to `output-token-budget-exceeded`, with
one content-free diagnostic. Persisted `activeDurationMs` was 504,772. Each
execution record has zero input, output, and total tokens and
`estimatedUsd: null`. The workspace `totalCostUsd` was persisted as `0`, but
provider-reported cost was unavailable; this value is not an actual
...
## Bounded matched-backend observation (#378)

On 2026-09-13, explicit provider-transmission authorization preceded this
twenty-first observation on merged revision
`04609d4583c7ed8c32ed779bc97ecb6d677c9ee0`, which included #376. It reused the
same private matched-role case and baseline-withheld v1 gate, with Anthropic
`claude-sonnet-4-5` as author and OpenAI `gpt-5.6-luna` as critic. The workspace
preflight was non-fixture and matched three rounds, a 1,200,000 ms request
duration, and a 1,200,000 ms maximum active duration. Both 20-second
user-session authentication probes passed.

The run made one Anthropic author attempt. It failed after cumulative
`activeDurationMs` 14,405 with code `unknown`, was non-retryable, and had no
`failureStage`, `failureReason`, or diagnostics. Execution input, output, and
total token counts were zero and `estimatedUsd` was null. The workspace
`totalCostUsd` persisted as `0`, but provider-reported cost was unavailable;
zero is not a measured provider cost.

No artifact, critic call, findings, readiness decision, review, adjudication,
approval, export, submission, release preparation, or release occurred. The
non-retryable run cannot resume, and its authorization is exhausted; another
run would require new explicit authorization. This observation does not
exercise or validate #376's corrected cumulative-usage acceptance in live use
and adds no product-quality evidence. Parity remains indeterminate; #75 stays
open, and release preparation under #250 remains blocked.

## Bounded matched-backend observation (#382)

On 2026-09-13, explicit provider-transmission authorization preceded this
twenty-second observation on revision
`349388de4820eca30543492d8ad1266199cdda3e`, which included #380. It reused the
same private matched-role case and baseline-withheld v1 gate, with Anthropic
...
The run made three Anthropic author attempts and no critic calls. Attempts one
and two failed with retryable `invalid-response`; attempt three failed with
the same code, non-retryably, with `factual-invariant-rejection` as its failure
stage and reason. The final cumulative `activeDurationMs` was 670,049. No
diagnostics were emitted. Provider-reported cost was unavailable.

No artifact, critic review, findings, readiness decision, review,
adjudication, approval, export, submission, release preparation, or release
occurred. The bounded attempt cap and authorization are exhausted; another
live run would require new explicit authorization. This adds no product-quality
...
- The earlier SDK-role comparison used the existing private manual CV as its
  ...
  twenty-first observation (#378) followed #376 but its single non-retryable
  author failure neither reached an artifact nor validated the corrected
...
The seventeenth observation under #362 reused the same matched-backend inputs
and gate with separate explicit provider-transmission authorization. Both
user-session authentication probes passed, but all three Anthropic author
executions failed with generic `invalid-response`; the final durable failure
stage and reason were `output-token-budget-exceeded`. It produced no artifact or
review boundary, and provider-reported cost was unavailable. It adds no
product-quality evidence.

The eighteenth observation under #364 reused the same matched-backend inputs
and gate after Claude reauthentication, with explicit provider-transmission
authorization before execution. Both user-session authentication probes passed.
All three Anthropic author executions failed with generic `invalid-response`;
attempt two was observed under `factual-invariant-rejection` with eight
content-free diagnostics, while the final durable failure stage and reason
were `output-token-budget-exceeded` with one diagnostic. Active duration was
504,772 ms; all execution token counts were zero and `estimatedUsd` was null.
...
is exhausted. The twenty-first observation (#378) is also recorded above. Its
