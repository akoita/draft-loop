...
The first portable-store slice establishes only the second boundary's identity
and SQLite lifecycle metadata. It does not yet put candidate source content in
that store or connect it to an application workflow.
...
- adding physical candidate source files or fetched source content;
- source identity, immutable versions, provenance, refresh, and duplicate
  handling;
...
- The store contains no source content in this slice, so it does not yet improve
  application retrieval or reduce repeated source import.
...
- Define managed physical-source intake and versioned provenance without
  retaining host paths in portable or provider-facing records.
