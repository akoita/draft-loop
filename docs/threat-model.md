...
- user-approved HTTPS URL ingestion and explicit refresh with bounded redirects
  and responses; and
...
created version. Complete bounded directory imports also persist a sensitive
canonical root binding and immutable membership hashes; partial and legacy
runtime-only imports do not. The application can explicitly approve one regular file of at
...
physical names, and URLs. The canonical directory root and successful file
origins are deliberate sensitive local-table exceptions; directory member rows
retain only SHA-256 hashes of normalized relative paths. A separate explicit local refresh follows only the
...
match. It creates no version, managed blob, journal event, or refresh-state
  mutation, replaces rather than retains the superseded path, and projects only
  source identity, status, and
binding time. Explicit URL intake and refresh reuse the bounded approved HTTPS
boundary; refresh fetches only immutable local original-URL provenance after
active-scope and retirement checks. Changed bytes append an exact version while
identical bytes are current, and failed approved attempts persist only a URL-free
...
and extraction failures fail closed. An explicit bounded add-members operation
can append unmatched accepted files as new managed sources and immutable members
...
it does not mutate the binding. The explicit application command reuses that
scan, performs stable per-member final verification, and atomically applies the
guarded root revision/origin update or a current-root no-op. Rename/removal decisions,
...
directory reconciliation and membership lifecycle; URL redirect history and
conditional requests; automatic duplicate resolution; indexes/retrieval;
...
origin-revision, and chronology guards. Its path-free `removed` result means
logical retirement, not physical deletion; an already retired member returns
`already-removed` without a write. Bytes, versions, bindings, observations,
journal state, and immutable membership remain unchanged.
...
retirements, or journal events. The explicit bounded applied operation
...
and returns a path-free partial result after a later member failure. Removal,
rename, reconciliation, automatic retirement/deletion, adapters, indexing, and
background refresh remain deferred; the separate root-rebind operation now
performs one guarded all-member origin/revision commit or a current-root no-op.
...
| T-005 | High / medium         | URL ingestion or refresh reaches a private service, follows an unsafe redirect, downloads excessive content, leaks a sensitive URL, or imports malicious markup.                        | Per-fetch approval, immutable-original-URL refresh, pre-network active-scope/kind/retirement checks, HTTPS-only parsing, literal and resolved-address checks, manual redirects, redirect/time/size caps, content-type restrictions, exact-byte integrity checks, opaque storage names, local-only immutable URL provenance, URL-free results/errors/observations, and atomic lifecycle rechecks before writes.                                                       | Test DNS rebinding and alternate address encodings, review resolver-to-connect and preflight-to-fetch races, keep active content inert, and define separate redirect-history and time-based readiness policy.                                       |
...
The portable CKB store path, selected directory roots, and manual-append paths
are local host configuration. None is part of logical identity, the portable
manifest, a provider request, or content-free audit/diagnostic data. A successful
individual file intake—including a file selected through directory intake—retains
the canonical verified origin path in the sensitive local binding table. A
complete directory import also retains its canonical root and hashed child
membership in sensitive local tables. That membership is a stable historical
mapping captured at binding or explicit append time; later source versioning, explicit origin
rebinding, or retirement does not rewrite it. The selected root and
manual-append path are runtime-only outside those deliberate local bindings.
The original filename is not a
managed physical name or persisted filename provenance, and selecting a
different path or basename does not change the source identity or label.
...
Directory intake treats `AGENTS.md`-like names and all other configuration-like
candidate files as inert content. Dot-prefixed entries/subtrees, unsupported
files, special entries, and child symlinks are skipped and counted rather than
opened or followed. The root is rejected if it is a symlink; child symlinks are
skipped. A complete import retains an immutable directory-root binding and
hashed source membership, but later filesystem changes cannot silently refresh
the CKB and partial or legacy runtime-only imports have no directory-root/
...
is read-only and the explicit application command performs the guarded rebind.
None of these operations infers
renames or complete removal reconciliation, and no operation
...
are guarded no-ops. The application rebind command now performs the explicit
scan-and-commit boundary with stable per-member final verification and generic
