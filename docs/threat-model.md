...
| T-013 | High / medium         | Plaintext managed bytes or a copied CKB remain after the original or workspace is deleted.                                             | Opaque names; logical identity separate from paths/URLs; explicit six-class retention policy; holds override expiry; sensitive bindings excluded from providers.                        | Same-user access remains possible; CKB deletion, backup/export/restore, and derived-data cleanup are unimplemented.              |
