...
| User-approved local file to portable CKB store        | Raw candidate bytes, stable store/source identity, local labels, checksums, and lifecycle metadata | Explicit add/manual append/bound-refresh/exact-byte-rebind operations; regular-file/type/20 MiB/extraction gates; stable copy for versions and read/hash verification for rebind; parent-linked changed versions; current refresh no-write; rebind changes only sensitive local origin state; opaque ID-derived no-replace copy; version-6 managed marker; path-free application results; best-effort restrictive permissions | Plaintext bytes and labels remain readable to same-user processes and backups; point-in-time inspection can race later changes; crashes or concurrency can leave unreferenced residue; background refresh, automatic moved-origin discovery, adapter controls, selection, retrieval, reconciliation, and broader lifecycle controls are not integrated |
...
| T-013 | High / medium         | Plaintext managed source bytes or a copied store remain readable after the user deletes the original file or application workspace and believes the material is gone.    | Logical identity is independent of host path; the sensitive canonical origin binding is local SQLite state, travels with a database copy without establishing portable continuity, and is excluded from descriptors, product projections, and provider-facing data; managed names are opaque; local permissions are restrictive where supported.                                        | Permissions are best-effort and do not stop a same-user process. Disclose that original/workspace deletion does not delete the CKB copy, the binding can become stale, SQLite-only backup is incomplete, and CKB deletion/export/restore and backup coverage remain unimplemented. |
...
- portable continuity for the local CKB origin binding, background refresh,
  persisted freshness/last-refresh, automatic moved-origin discovery and
  adapter-level rebind controls,
