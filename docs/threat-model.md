...
| T-016 | Medium / high         | A crash or concurrent writer leaves an unreferenced blob or metadata without verified bytes.                                           | File-first/database-second publication, no-replace targets, managed markers, ordinary cleanup, count-only inventory, prospective journal, store-wide fenced writer lease.                | A crash can still leave an interrupted journaled write; #162 defines deterministic recovery, while legacy and unjournaled entries remain unknown. |
...
| T-018 | Medium / medium       | A journal entry, matching bytes, or staging-shaped name is mistaken for ownership.                                                     | Journal has opaque operation events but no cleanup/approval field; no retroactive legacy claims or adoption by bytes/shape; cleanup-capable work is fenced by a store-wide lease.       | Add visible approval before any reconciliation or cleanup.                                                                       |
