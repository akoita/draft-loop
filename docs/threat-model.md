...
| Application to model provider | Candidate sources, context, prompts, and drafts | `DataExposurePolicy` and `assertDataExposureAllowed` in `packages/providers/src/index.ts`; provider/model identity and bounded run budgets | The full desktop preflight is not yet cross-platform validated; provider retention and training behavior remain external facts |
...
| T-002 | High / high | Sensitive candidate or employer material is sent to an unapproved provider, model, or endpoint without informed acknowledgement. | Provider requests carry `DataExposurePolicy`; provider allowlist and sensitive-data acknowledgement fail before the SDK call. | Complete the desktop preflight showing data class, provider/model, endpoint, retention preference, budget, and transmission scope; record cross-platform acceptance. |
...
Runtime controls are deny-by-default provider egress, explicit URL approval,
...
- the desktop credential and provider preflight needs supported-platform
  acceptance, including OS-store availability and fallback behavior;
