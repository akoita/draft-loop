...
moves/disappears, is not automatically updated or rebound, and is never
provider-facing. An
...
rebinds or persists freshness. Background refresh, persisted freshness/last-refresh,
...
| User-approved local file to portable CKB store        | Raw candidate bytes, stable store/source identity, local labels, checksums, and lifecycle metadata | Explicit add/manual append/bound-refresh operations; repeated regular-file/type/20 MiB/extraction/stable-copy gates; parent-linked changed versions; current refresh no-write; opaque ID-derived no-replace copy; version-6 managed marker; local-only path binding; path-free, non-persisted origin status; best-effort restrictive permissions | Plaintext bytes and labels remain readable to same-user processes and backups; point-in-time inspection can race later changes; crashes or concurrency can leave unreferenced residue; background refresh, rebind, selection, retrieval, reconciliation, and lifecycle controls are not integrated |
...
remembered origin ------> explicit status / changed-byte refresh
