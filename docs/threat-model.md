...
| T-009 | High / medium         | Retrieval mixes workspaces, serves stale chunks, or retains derived data after source deletion.                                        | Workspace-scoped legacy queries; ADR 0008 assigns exact-version active indexes to each CKB and content-free traces to workspaces.                                                        | Implement and prove selection-only fan-out, deletion, rebuild, drift, provenance, and isolation before CKB cutover or vector/hybrid retrieval. |
