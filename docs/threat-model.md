...
directory membership/incremental refresh, URL redirect history/conditional requests, automatic
...
| Application to approved local directory               | Candidate files, local paths, traversal reachability, and aggregate intake size                    | Explicit local approval; real non-symlink root outside the CKB store; canonical realpath containment; lexical recursive traversal; depth, scanned-entry, accepted-file, aggregate-byte, and 20 MiB per-file limits; lstat mutation checks; no child-symlink following or special-entry opens; extraction before writes; skipped-entry counts; independent managed file sources; path-free generic failures                                                                                                           | The selected directory root remains runtime-only and has no membership or directory-root binding; accepted child files retain ordinary file-origin bindings; same-user mutation races remain possible despite repeated checks; counts reveal limited directory shape; additions/removals after intake are not tracked and incremental refresh is not implemented                                                                                |
...
the canonical verified origin path in the sensitive local binding table. The
selected directory root and manual-append path are runtime-only and are not
remembered as directory or append bindings. The original filename is not a
...
opened or followed. The directory itself is not retained as a binding or source
membership, so later filesystem changes cannot silently refresh the CKB.
...
- portable continuity for the local CKB origin binding, background refresh,
  ...
  directory membership/incremental refresh, URL redirect-history/conditional-request readiness,
