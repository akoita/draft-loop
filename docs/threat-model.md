...
| T-018 | Medium / medium       | A journal entry, matching bytes, or staging-shaped name is mistaken for ownership.                                                     | Recovery requires ownership metadata, a durable newer-generation claim, expected integrity, persisted staging identity, deterministic target, and current lease; bytes or shape alone never suffice. | Preserve any record or artifact that fails ownership checks; require visible approval for retention or user-requested deletion. |
