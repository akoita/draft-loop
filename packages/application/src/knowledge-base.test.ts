...
describe("candidate knowledge store application service", () => {
  ...
  it("previews a directory-root rebind as a read-only exact membership check", async () => {
    const boundDirectory = join(temporaryParent, "bound-directory");
    const candidateDirectory = join(temporaryParent, "candidate-directory");
    const relativePath = join("nested", "evidence.txt");
    const boundPath = join(boundDirectory, relativePath);
    ...
    await writeFile(candidatePath, "same evidence", "utf8");
    const generatedIds = [
      "store-uuid",
      "default-ckb-uuid",
      "source-uuid",
    ...
    const snapshot = async () => {
      ...
      try {
        return {
          ...
          versions: await store.listCandidateKnowledgeSourceVersions(
            "default-ckb-uuid",
            "source-uuid",
          ),
          origin: await store.getCandidateKnowledgeSourceOriginBinding(
            "default-ckb-uuid",
            "source-uuid",
          ),
          observations: await store.getCandidateKnowledgeSourceRefreshObservation(
            "default-ckb-uuid",
            "source-uuid",
          ),
    ...
    await rm(boundDirectory, { recursive: true, force: true });

    const ready = await service.previewKnowledgeSourceDirectoryRootRebind({
      storeRoot,
      knowledgeBaseId: "default-ckb-uuid",
      directoryId: "directory-binding",
    ...
    expect(JSON.stringify(ready)).not.toContain(temporaryParent);
  ...
  it("rejects root-rebind candidates with missing, extra, changed, or duplicate membership", async () => {
    ...
    const missingDirectory = join(temporaryParent, "missing-candidate");
    const extraDirectory = join(temporaryParent, "extra-candidate");
    const changedDirectory = join(temporaryParent, "changed-candidate");
    const relativePath = join("nested", "evidence.txt");
    const boundPath = join(boundDirectory, relativePath);
    const otherBoundPath = join(otherBoundDirectory, relativePath);
    await mkdir(dirname(boundPath), { recursive: true });
    await mkdir(dirname(otherBoundPath), { recursive: true });
    await writeFile(boundPath, "same evidence", "utf8");
    await writeFile(otherBoundPath, "other evidence", "utf8");
    await mkdir(missingDirectory);
    const extraPath = join(extraDirectory, relativePath);
    await mkdir(dirname(extraPath), { recursive: true });
    await writeFile(extraPath, "same evidence", "utf8");
    await writeFile(join(extraDirectory, "extra.txt"), "new evidence", "utf8");
    const changedPath = join(changedDirectory, relativePath);
    await mkdir(dirname(changedPath), { recursive: true });
    await writeFile(changedPath, "changed evidence", "utf8");
    const ids = [
      "store-uuid",
      "default-ckb-uuid",
      "source-uuid",
    ...
    await expect(
      service.importKnowledgeSourceDirectory({
        storeRoot,
        knowledgeBaseId: "default-ckb-uuid",
        directoryPath: otherBoundDirectory,
      }),
    ).resolves.toMatchObject({ status: "complete", directoryId: "other-directory" });
    const callCountBeforePreviews = generateId.mock.calls.length;
    const failureMessage =
      "The selected candidate knowledge source directory root rebind preview could not be completed.";
    ...
    expect(generateId).toHaveBeenCalledTimes(callCountBeforePreviews);
  });

  it("fails closed when a root-rebind scan returns duplicate accepted paths", async () => {
    const sourcePath = "/candidate/current.txt";
    ...
    const handle = {
      ...
      findCandidateKnowledgeDirectoryBinding: vi.fn(async () => undefined),
      listCandidateKnowledgeDirectoryMembers: vi.fn(async () => [member]),
      findCandidateKnowledgeDirectoryMemberByCandidateRootAndPath: vi.fn(async () => member),
      ...
      getManagedCandidateKnowledgeFilePath: vi.fn(async () => "/store/managed-file"),
    ...
    const service = createCandidateKnowledgeStoreService({
      ...
      realpath: vi.fn(async (path: string) => path) as never,
      now: () => createdAt,
    });
    await expect(
      service.previewKnowledgeSourceDirectoryRootRebind({
    ...
    vi.mocked(handle.getManagedCandidateKnowledgeFilePath).mockResolvedValueOnce(undefined);
    await expect(
      service.previewKnowledgeSourceDirectoryRootRebind({
        storeRoot: "/store",
        knowledgeBaseId: "default-ckb-uuid",
  ...
  it("previews exact-integrity moved candidates with ambiguity and lifecycle guards", async () => {
    const directoryPath = join(temporaryParent, "moved-candidate-directory");
    await mkdir(directoryPath);
    const paths = {
      uniqueA: join(directoryPath, "a-unique.txt"),
      oneToMany: join(directoryPath, "b-one.txt"),
      manyFirst: join(directoryPath, "c-many-first.txt"),
      manySecond: join(directoryPath, "d-many-second.txt"),
      changed: join(directoryPath, "e-changed.txt"),
      uniqueF: join(directoryPath, "f-unique.txt"),
      retired: join(directoryPath, "g-retired.txt"),
      conflict: join(directoryPath, "h-conflict.txt"),
    };
    await writeFile(paths.uniqueA, "unique-a", "utf8");
    await writeFile(paths.oneToMany, "shared-one", "utf8");
    await writeFile(paths.manyFirst, "shared-many", "utf8");
    await writeFile(paths.manySecond, "shared-many", "utf8");
    await writeFile(paths.changed, "before", "utf8");
    await writeFile(paths.uniqueF, "unique-f", "utf8");
    ...
    await rm(paths.uniqueA);
    await rm(paths.oneToMany);
    await rm(paths.manyFirst);
    await rm(paths.manySecond);
    await rm(paths.uniqueF);
    await rm(paths.retired);
    await rm(paths.conflict);
    await writeFile(paths.changed, "after", "utf8");
    const conflictReplacement = join(temporaryParent, "moved-conflict.txt");
    await writeFile(conflictReplacement, "conflict", "utf8");
    now = changedAt;
    ...
    await writeFile(join(directoryPath, "m-a.txt"), "unique-a", "utf8");
    await writeFile(join(directoryPath, "m-b-one.txt"), "shared-one", "utf8");
    await writeFile(join(directoryPath, "m-b-two.txt"), "shared-one", "utf8");
    await writeFile(join(directoryPath, "m-f.txt"), "unique-f", "utf8");
    await writeFile(join(directoryPath, "m-many.txt"), "shared-many", "utf8");
    await writeFile(join(directoryPath, "m-g-retired.txt"), "retired", "utf8");
    await writeFile(join(directoryPath, "m-h-conflict.txt"), "conflict", "utf8");
    ...
    const result = await service.previewKnowledgeSourceDirectoryMovedCandidates({
      storeRoot,
      knowledgeBaseId: "default-ckb-uuid",
      directoryId: "directory-binding",
    });
    expect(result).toEqual({
      ...
      candidates: [
        ...
        { sourceId: "f-source", status: "moved-candidate" },
      ],
      candidateCount: 2,
      newSourceCount: 7,
      scannedEntryCount: 8,
    ...
    expect(JSON.stringify(result)).not.toContain(sha256("unique-a"));
    expect(JSON.stringify(result)).not.toContain("unique-a");
    expect(JSON.stringify(result)).not.toContain("a-version");
    expect(ingestDirectory).toHaveBeenCalledOnce();
    expect(generateId).toHaveBeenCalledTimes(19);
    expect(await snapshot()).toEqual(before);
  ...
  it("fails closed when a moved candidate latest managed backing is unavailable", async () => {
    ...
    const close = vi.fn(async () => undefined);
    const getManagedCandidateKnowledgeFilePath = vi.fn(async () => undefined);
    const createManagedCandidateKnowledgeFileSource = vi.fn();
    const upsertCandidateKnowledgeDirectoryRefreshObservations = vi.fn();
    const handle = {
      getCandidateKnowledgeBase: vi.fn(async () => ({
        id: "default-ckb-uuid",
        displayName: "Evidence",
      ...
      listCandidateKnowledgeDirectoryMembers: vi.fn(async () => [member]),
      findCandidateKnowledgeDirectoryMemberByPath: vi.fn(async () => undefined),
      getCandidateKnowledgeDirectoryMemberOriginRelation: vi.fn(async () => ({
        directoryId: member.directoryId,
        knowledgeBaseId: member.knowledgeBaseId,
        sourceId: member.sourceId,
      ...
      listCandidateKnowledgeSourceVersions: vi.fn(async () => [version]),
      getCandidateKnowledgeSourceRetirement: vi.fn(async () => undefined),
      getManagedCandidateKnowledgeFilePath,
      createManagedCandidateKnowledgeFileSource,
      upsertCandidateKnowledgeDirectoryRefreshObservations,
      close,
    } as unknown as CandidateKnowledgeStoreHandle;
    const ingestDirectory = vi.fn(async () => ({
      sources: [normalized],
      scannedEntryCount: 1,
      discoveredFileCount: 1,
      skippedEntryCount: 0,
    }));
    const generateId = vi.fn(() => "unexpected-id");
    const service = createCandidateKnowledgeStoreService({
      generateId,
      ingestDirectory: ingestDirectory as never,
      lstat: vi.fn(async () => ({
    ...
    expect(ingestDirectory).toHaveBeenCalledOnce();
    expect(getManagedCandidateKnowledgeFilePath).toHaveBeenCalledWith(
      "default-ckb-uuid",
      "source-uuid",
      "version-uuid",
    );
    expect(generateId).not.toHaveBeenCalled();
    expect(createManagedCandidateKnowledgeFileSource).not.toHaveBeenCalled();
    expect(upsertCandidateKnowledgeDirectoryRefreshObservations).not.toHaveBeenCalled();
    expect(close).toHaveBeenCalledOnce();
  });

  it("applies one directory member move and repeats as a guarded current no-op", async () => {
    const directoryPath = join(temporaryParent, "member-move-directory");
    const initialPath = join(directoryPath, "old.txt");
    const movedPath = join(directoryPath, "moved.txt");
    const content = "stable member content";
    await mkdir(directoryPath);
    ...
    const snapshot = async () => {
      ...
      try {
        return {
          source: await store.getCandidateKnowledgeSource("default-ckb-uuid", "source-uuid"),
          versions: await store.listCandidateKnowledgeSourceVersions(
            "default-ckb-uuid",
            "source-uuid",
          ),
          ...
          revision: await store.getCandidateKnowledgeDirectoryMemberCurrentRevision(
            "default-ckb-uuid",
            "directory-binding",
            "source-uuid",
          ),
    ...
    const before = await snapshot();
    const generatedIdCount = generateId.mock.calls.length;
    ingestDirectory.mockClear();
    now = changedAt;

    const moved = await service.applyKnowledgeSourceDirectoryMemberMove({
      storeRoot,
      knowledgeBaseId: "default-ckb-uuid",
      directoryId: "directory-binding",
      sourceId: "source-uuid",
      options: { maxScannedEntries: 32 },
    });
    ...
    expect(JSON.stringify(moved)).not.toContain(directoryPath);
    expect(JSON.stringify(moved)).not.toContain("old.txt");
    expect(JSON.stringify(moved)).not.toContain("moved.txt");
    expect(JSON.stringify(moved)).not.toContain(sha256(content));
    expect(JSON.stringify(moved)).not.toContain("version-uuid");
    expect(ingestDirectory).toHaveBeenCalledOnce();
    expect(ingestDirectory.mock.calls[0]?.[1]).toEqual({ maxScannedEntries: 32 });
    expect(generateId).toHaveBeenCalledTimes(generatedIdCount);
    const afterMove = await snapshot();
    expect(afterMove.source).toEqual(before.source);
    expect(afterMove.versions).toEqual(before.versions);
    expect(afterMove.observation).toEqual(before.observation);
    expect(afterMove.retirement).toEqual(before.retirement);
    expect(afterMove.inventory).toEqual(before.inventory);
    expect(afterMove.members).toEqual([
      {
        directoryId: "directory-binding",
        knowledgeBaseId: "default-ckb-uuid",
        sourceId: "source-uuid",
        relativePathHash: sha256("moved.txt"),
      },
    ]);
    expect(afterMove.revision).toMatchObject({
      revision: 2,
      relativePathHash: sha256("moved.txt"),
      boundAt: changedAt,
    });
    expect(afterMove.origin).toEqual({
      sourceId: "source-uuid",
      originPath: movedPath,
      boundAt: changedAt,
    });

    ingestDirectory.mockClear();
    const current = await service.applyKnowledgeSourceDirectoryMemberMove({
      storeRoot,
      knowledgeBaseId: "default-ckb-uuid",
      directoryId: "directory-binding",
      sourceId: "source-uuid",
    ...
    expect(ingestDirectory).toHaveBeenCalledOnce();
    expect(generateId).toHaveBeenCalledTimes(generatedIdCount);
    expect(await snapshot()).toEqual(afterMove);

    const malformedBaseHandle = await openCandidateKnowledgeStore(storeRoot);
    const malformedHandle = {
      ...malformedBaseHandle,
      moveManagedCandidateKnowledgeDirectoryMember: vi.fn(async () => ({ moved: true })),
    } as unknown as CandidateKnowledgeStoreHandle;
    const malformedService = createCandidateKnowledgeStoreService({
      open: vi.fn(async () => malformedHandle),
      ingestDirectory: ingestDirectory as never,
      now: () => now,
    });
    await expect(
      malformedService.applyKnowledgeSourceDirectoryMemberMove({
        storeRoot,
        knowledgeBaseId: "default-ckb-uuid",
        directoryId: "directory-binding",
        sourceId: "source-uuid",
    ...
    const semanticBaseHandle = await openCandidateKnowledgeStore(storeRoot);
    const semanticHandle = {
      ...semanticBaseHandle,
      moveManagedCandidateKnowledgeDirectoryMember: vi.fn(async () => ({
        member: {
          directoryId: "directory-binding",
          knowledgeBaseId: "default-ckb-uuid",
          sourceId: "source-uuid",
          relativePathHash: sha256("semantically-fake.txt"),
        },
        revision: {
          directoryId: "directory-binding",
          knowledgeBaseId: "default-ckb-uuid",
    ...
    const semanticService = createCandidateKnowledgeStoreService({
      open: vi.fn(async () => semanticHandle),
      ingestDirectory: ingestDirectory as never,
      now: () => now,
    });
    await expect(
      semanticService.applyKnowledgeSourceDirectoryMemberMove({
        storeRoot,
        knowledgeBaseId: "default-ckb-uuid",
        directoryId: "directory-binding",
        sourceId: "source-uuid",
    ...
    const raceBaseHandle = await openCandidateKnowledgeStore(storeRoot);
    const raceHandle = {
      ...raceBaseHandle,
      moveManagedCandidateKnowledgeDirectoryMember: vi.fn(async (input) => {
        await writeFile(movedPath, "changed immediately before verification", "utf8");
        return raceBaseHandle.moveManagedCandidateKnowledgeDirectoryMember(input);
      }),
    } as unknown as CandidateKnowledgeStoreHandle;
    const raceService = createCandidateKnowledgeStoreService({
      open: vi.fn(async () => raceHandle),
      ingestDirectory: ingestDirectory as never,
      now: () => now,
    });
    await expect(
      raceService.applyKnowledgeSourceDirectoryMemberMove({
        storeRoot,
        knowledgeBaseId: "default-ckb-uuid",
        directoryId: "directory-binding",
        sourceId: "source-uuid",
    ...
    const staleRootBaseHandle = await openCandidateKnowledgeStore(storeRoot);
    const currentRootRevision =
      await staleRootBaseHandle.getCandidateKnowledgeDirectoryCurrentRootRevision(
        "default-ckb-uuid",
        "directory-binding",
      );
    if (currentRootRevision === undefined) throw new Error("expected current root revision");
    const staleRootHandle = {
      ...staleRootBaseHandle,
      getCandidateKnowledgeDirectoryCurrentRootRevision: vi.fn(async () => ({
        ...currentRootRevision,
        revision: currentRootRevision.revision + 1,
      })),
    } as unknown as CandidateKnowledgeStoreHandle;
    const staleRootService = createCandidateKnowledgeStoreService({
      open: vi.fn(async () => staleRootHandle),
      ingestDirectory: ingestDirectory as never,
      now: () => now,
    });
    await expect(
      staleRootService.applyKnowledgeSourceDirectoryMemberMove({
        storeRoot,
        knowledgeBaseId: "default-ckb-uuid",
        directoryId: "directory-binding",
        sourceId: "source-uuid",
    ...
    const staleMemberBaseHandle = await openCandidateKnowledgeStore(storeRoot);
    const currentMemberRevision =
      await staleMemberBaseHandle.getCandidateKnowledgeDirectoryMemberCurrentRevision(
        "default-ckb-uuid",
        "directory-binding",
        "source-uuid",
      );
    if (currentMemberRevision === undefined) throw new Error("expected current member revision");
    const staleMemberHandle = {
      ...staleMemberBaseHandle,
      getCandidateKnowledgeDirectoryMemberCurrentRevision: vi.fn(async () => ({
        ...currentMemberRevision,
        revision: currentMemberRevision.revision + 1,
      })),
    } as unknown as CandidateKnowledgeStoreHandle;
    const staleMemberService = createCandidateKnowledgeStoreService({
      open: vi.fn(async () => staleMemberHandle),
      ingestDirectory: ingestDirectory as never,
      now: () => now,
    });
    await expect(
      staleMemberService.applyKnowledgeSourceDirectoryMemberMove({
        storeRoot,
        knowledgeBaseId: "default-ckb-uuid",
        directoryId: "directory-binding",
        sourceId: "source-uuid",
    ...
    await writeFile(movedPath, "changed content", "utf8");
    await expect(
      service.applyKnowledgeSourceDirectoryMemberMove({
        storeRoot,
        knowledgeBaseId: "default-ckb-uuid",
        directoryId: "directory-binding",
        sourceId: "source-uuid",
  ...
  it("rejects ambiguous, conflicted, retired, and unavailable directory member moves", async () => {
    const directoryPath = join(temporaryParent, "member-move-guards");
    const initialPath = join(directoryPath, "old.txt");
    const firstCandidatePath = join(directoryPath, "candidate-a.txt");
    const secondCandidatePath = join(directoryPath, "candidate-b.txt");
    const outsidePath = join(temporaryParent, "member-move-outside.txt");
    const content = "guarded member content";
    await mkdir(directoryPath);
    await writeFile(initialPath, content, "utf8");
    const ids = [
      "store-uuid",
    ...
    await service.initializeStore({ storeRoot });
    await service.importKnowledgeSourceDirectory({
      storeRoot,
      knowledgeBaseId: "default-ckb-uuid",
      directoryPath,
    });
    ...
    await rm(initialPath);
    await writeFile(firstCandidatePath, content, "utf8");
    await writeFile(secondCandidatePath, content, "utf8");
    const beforeAmbiguous = await snapshot();
    const generatedIdCount = generateId.mock.calls.length;
    await expect(
      service.applyKnowledgeSourceDirectoryMemberMove({
        storeRoot,
    ...
    expect(await snapshot()).toEqual(beforeAmbiguous);
    expect(generateId).toHaveBeenCalledTimes(generatedIdCount);

    await rm(firstCandidatePath);
    await rm(secondCandidatePath);
    await writeFile(initialPath, content, "utf8");
    ...
    const baseHandle = await openCandidateKnowledgeStore(storeRoot);
    const unavailableHandle = {
      ...baseHandle,
      getManagedCandidateKnowledgeFilePath: vi.fn(async () => undefined),
    } as unknown as CandidateKnowledgeStoreHandle;
    const unavailableService = createCandidateKnowledgeStoreService({
      open: vi.fn(async () => unavailableHandle),
      ingestDirectory: ingestDirectory as never,
      now: () => now,
    });
    const beforeUnavailable = await snapshot();
    await expect(
      unavailableService.applyKnowledgeSourceDirectoryMemberMove({
        storeRoot,
        knowledgeBaseId: "default-ckb-uuid",
        directoryId: "directory-binding",
        sourceId: "source-uuid",
    ...
    expect(await snapshot()).toEqual(beforeUnavailable);
    ...
    await writeFile(initialPath, content, "utf8");
    const beforeRetired = await snapshot();
    await expect(
      service.applyKnowledgeSourceDirectoryMemberMove({
        storeRoot,
        knowledgeBaseId: "default-ckb-uuid",
    ...
    expect(await snapshot()).toEqual(beforeRetired);
  });
